...
def fn_get_hull_polygons(dict_params):
    ...
    for i in range(int_last_valid_cluster_index + 1):
        ...
        gdf_new = gpd.GeoDataFrame(pd.DataFrame(dict_bridge_hull, index=[0]),
                   geometry="geometry",
                   crs=str_lambert)
        
        gdf_bridge_hulls = pd.concat([gdf_bridge_hulls,gdf_new])
    ...
    gdf_bridge_hulls.reset_index(drop=True)
...
def fn_polygonize_point_groups(str_las_input_directory, str_output_dir, int_class, flt_epsilon, int_min_samples):
    ...
    # minimum area of polygon to retain - 2023.07.05
    flt_min_area = 5
    ...
    if len(list_files_with_points) > 0:
        ...
        # ---------------
        # drop all the polygons that do not meet the minimum area (flt_min_area) requirements
        gdf_bridge_poly = gdf_merge_polygons
        
        # compute the area of each polygon in the geodataframe
        gdf_bridge_poly['area'] = gdf_bridge_poly.area
        
        # drop all values where area less than threshold
        gdf_bridge_area_min = gdf_bridge_poly.drop(gdf_bridge_poly[gdf_bridge_poly['area'] < flt_min_area].index)
        ...
        gdf_bridge_area_min.to_file(str_file_shp_to_write)
        ...
        gdf_bridge_area_min.to_file(str_file_gpkg_to_write, driver='GPKG')
        print("+-----------------------------------------------------------------+")
